...
1. First, get the node names using `kubectl get node`:
    ...
    ```bash
    10.0.10.35   Ready    node    172m   v1.23.4
    10.0.10.46   Ready    node    172m   v1.23.4
    10.0.10.86   Ready    node    171m   v1.23.4
    ```
...
2. Now check the current pod allocation using the detailed pod information:
    ...
    sample-domain1-admin-server      1/1     Running   0          11m     10.244.1.5     10.0.10.86   <none>           <none>
    sample-domain1-managed-server1   1/1     Running   0          10m     10.244.1.6     10.0.10.86   <none>           <none>
    sample-domain1-managed-server2   1/1     Running   0          9m48s   10.244.0.133   10.0.10.46   <none>           <none>
    sample-domain1-managed-server3   1/1     Running   0          8m41s   10.244.0.10    10.0.10.35   <none>           <none>
    ```
...
Knowing the node names, select one which you want to be empty. In this example, this node will be: `10.0.10.35`
...
1. Label the other nodes. The label can be any string, but let's use `wlservers1` and `wlservers2`. Execute the
    ...
    ```bash
    $ kubectl label nodes 10.0.10.86  wlservers1=true
    node/10.0.10.86 labeled
    $ kubectl label nodes 10.0.10.46 wlservers2=true
    node/10.0.10.46 labeled
    ```
...
5. The operator according to the changes will start to relocate servers. Poll the pod information and wait until the expected result:
    ...
    ```bash
    NAME                             READY   STATUS    RESTARTS   AGE     IP             NODE         NOMINATED NODE   READINESS GATES
    sample-domain1-admin-server      1/1     Running   0          3m11s   10.244.0.135   10.0.10.46   <none>           <none>
    sample-domain1-managed-server1   1/1     Running   0          2m23s   10.244.1.9     10.0.10.86   <none>           <none>
    sample-domain1-managed-server2   1/1     Running   0          100s    10.244.1.10    10.0.10.86   <none>           <none>
    sample-domain1-managed-server3   1/1     Running   0          56s     10.244.0.136   10.0.10.46   <none>           <none>
    ```
...
1. To delete the node assignment, delete the node's label using the `kubectl label nodes <nodename> <labelname>-` command but replace the node name properly:
    ```bash
    $ kubectl label nodes 10.0.10.46 wlservers2-
    node/10.0.10.46 unlabeled
    $ kubectl label nodes 10.0.10.86 wlservers1-
    node/10.0.10.86 unlabeled
    ```
